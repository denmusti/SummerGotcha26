...
export async function POST(request) {
  ...
  // Authenticatie: admin wachtwoord, marshall wachtwoord, of toegangscode van de schutter
  const { data: stats } = await supabase.from('stats').select('wachtwoord').eq('id', 1).single();
  const isAdmin = body.wachtwoord && body.wachtwoord === stats?.wachtwoord;

  let isMarshall = false;
  if (!isAdmin && body.wachtwoord) {
    const { data: m } = await supabase.from('marshalls').select('id').eq('wachtwoord', body.wachtwoord).single();
    isMarshall = !!m;
  }
  ...
  if (!isAdmin && !isMarshall && !isDeelnemer) {
    return Response.json({ error: 'Ongeldig wachtwoord' }, { status: 401 });
  }
