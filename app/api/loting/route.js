...
async function checkWachtwoord(supabase, wachtwoord) {
  ...
  if (stats?.wachtwoord === wachtwoord) return true;
...
export async function POST(request) {
  ...
  // Loting genereren
  if (actie === 'genereer') {
    // Blokkeer als spel al gestart is (tenzij testModus)
    if (!testModus) {
      const { data: stats } = await supabase.from('stats').select('start_datum').eq('id', 1).single();
      if (stats?.start_datum && new Date() >= new Date(stats.start_datum)) {
        return Response.json({ error: 'Het spel is al gestart — de loting kan niet meer gewijzigd worden.' }, { status: 403 });
      }
    }
  ...
  if (actie === 'aanpassing') {
    const { schutter_id, nieuw_doelwit_id, marshall_id } = body;
    // schutter_id = deelnemer 1, nieuw_doelwit_id = deelnemer 2

    if (!marshall_id) {
      return Response.json({ error: 'Marshall ID is verplicht' }, { status: 400 });
    }

    // Haal marshall op via marshall_id
    const { data: marshallData, error: marshallErr } = await supabase
      .from('marshalls').select('id, naam, aanpassingen').eq('id', Number(marshall_id)).single();
