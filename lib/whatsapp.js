...
export async function stuurKillPubliek(telefoons, aantalLevenden) {
  return stuurNaarLijst(telefoons, { "1": `Er is een nieuwe kill! Nog ${aantalLevenden} spelers actief` });
}
...
export async function stuurStartBericht(telefoon, naam, toegangscode, killcode) {
  return stuurTwilioTemplate(telefoon, { "1": `Start! Welkom ${naam}. Code: ${toegangscode} - Killcode: ${killcode} - App: ${APP_URL}/mijn-doelwit` });
}
